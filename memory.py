...
async def nudge_user(user_id: int, body: NudgeRequest):
    spending_intent = body.spending_intent
    # Generate embedding for the spending intent
    ...
    # Check for similarity score > 0.75
    if results.get("distances") and results["distances"][0] and results["distances"][0][0] < (1 - 0.75):
        past_memory = results["documents"][0][0]
        return {"message": f"⚠️ Heads up! Based on your past memory: '{past_memory}', you may regret this purchase. Want to think twice?"}
    return {"message": "✅ You're good to go. No red flags from your past spending behavior."}
